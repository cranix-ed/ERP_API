...
-- 8️ TÀI KHOẢN
------------------------------------------------------------
INSERT INTO TaiKhoan (MaNV, Pass, HoTen, QuyenTruyCap)
VALUES
(1, N'Anh@123', N'Nguyễn Minh Anh', N'User'),
(2, N'Mai@123', N'Lê Thị Mai', N'Manager'),
(3, N'Dung@123', N'Phạm Văn Dũng', N'Admin');
GO
...
CREATE TRIGGER trg_SyncNhanVien_To_NhaCungCap
ON dbo.NhanVien
AFTER INSERT
AS
BEGIN
    ...
    INSERT INTO [QuanLyNhaCungCap].dbo.NhanVien (HoTen, GioiTinh, NgaySinh, DiaChi, CCCD, STK, NganHang)
    SELECT 
        i.HoTen, 
        i.GioiTinh, 
        i.NgaySinh, 
...
-- =============================================
-- TRIGGER CẬP NHẬT TỒN KHO KHI THÊM PHIẾU NHẬP HÀNG
-- =============================================
GO
...
BEGIN
    ...
    UPDATE NVL
    SET NVL.SoLuong = NVL.SoLuong + i.SoLuong
...
-- =============================================
-- TRIGGER CẬP NHẬT TỒN KHO KHI SỬA PHIẾU NHẬP HÀNG
-- (Giả định người dùng có thể sửa lại số lượng trong phiếu nhập)
-- =============================================
...
BEGIN
    ...
    -- Giảm số lượng tồn cũ
    UPDATE NVL
    SET NVL.SoLuong = NVL.SoLuong - d.SoLuong
    FROM NguyenVatLieu NVL
    INNER JOIN deleted d ON NVL.MaNVL = d.MaNVL;

    -- Tăng lại theo số lượng mới
    UPDATE NVL
    SET NVL.SoLuong = NVL.SoLuong + i.SoLuong
    FROM NguyenVatLieu NVL
    INNER JOIN inserted i ON NVL.MaNVL = i.MaNVL;
...
-- =============================================
-- TRIGGER CẬP NHẬT TỒN KHO KHI XÓA PHIẾU NHẬP HÀNG
-- =============================================
CREATE TRIGGER trg_CapNhatTonKho_Delete
ON PhieuNhapHang
AFTER DELETE
AS
BEGIN
